...
struct NotifySettingsSelectorView: View {
    ...
    var body: some View {
        ZStack {
            ...
            HStack {
                Image(isActive ? "ic_message_active" : "ic_message_unactive")

                VStack(alignment: .leading, spacing: 3) {
                    Text(NSLocalizedString("prohibit_incoming_messages", comment: "hint"))
                        .foregroundColor(isActive ? ColorList.white.color : ColorList.white_50.color)
                        .font(MyFont.getFont(.NORMAL, 16))
                    Text(NSLocalizedString(isActive ? "active" : "deactive", comment: "text"))
                        .foregroundColor(isActive ? ColorList.light_blue.color : ColorList.pink.color)
                        .font(MyFont.getFont(.NORMAL, 16))
                }
