...
struct MainOnlineBottomSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Title(textColor: ColorList.white.color, text: "online", textSize: 32, paddingV: 0, paddingH: 24)
            ...
            ForEach(mediator.itemList, id: \.id) { item in
                DarkBottomSheetItem(text: item.name, isSelect: equals(item: item.name)) { name in
                    ...
                    UserDataHolder.setSettings(type: .SEARCH_ONLINE, value: item.type.rawValue)
